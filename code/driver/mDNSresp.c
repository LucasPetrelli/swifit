...
int ICACHE_FLASH_ATTR mDNSresp_addhost(char *hn, struct ip_addr* ip)
{
	...
	h->hostname = (char *)os_zalloc(strlen(hn)+1);
	strcpy(h->hostname,hn);
	h->ip.addr = ip->addr;
